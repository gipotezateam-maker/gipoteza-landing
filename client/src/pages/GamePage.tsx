...
export default function GamePage() {
  ...
  function pick(i: number) {
    ...
    setShowHint(false);
    try { (window as any).ym(101798936, 'reachGoal', `game_scene_${week}`, { choice: opt.label, good: isGood }); } catch{}
    try { (window as any)._tmr.push({type:'reachGoal',id:'3766918',goal:`game_scene_${week}`}); } catch{}
    setTimeout(() => setShowRes(true), 200);
  }

  function next() {
    if (isLast || lives <= 0) { setScreen("end"); try { (window as any).ym(101798936, 'reachGoal', 'game_finish', { score, romi }); } catch{} try { (window as any)._tmr.push({type:'reachGoal',id:'3766918',goal:'game_finish'}); } catch{} return; }
  ...
  async function submitForm(e: React.FormEvent) {
    ...
    try { (window as any).ym(101798936, 'reachGoal', 'game_lead_submit'); } catch{}
    ...
    <div style={{
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", padding: "32px 20px 28px", animation: "fadeUp 0.6s ease" }}>
        ...
        <div style={{ width: "100%", maxWidth: 560, display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <button className="start-btn" onClick={() => { setScreen("game"); try { (window as any).ym(101798936, 'reachGoal', 'game_start'); } catch{} try { (window as any)._tmr.push({type:'reachGoal',id:'3766918',goal:'game_start'}); } catch{} }}>Начать игру</button>
